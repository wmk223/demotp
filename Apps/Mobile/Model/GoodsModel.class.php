...
class GoodsModel extends BaseModel {
	...
	public function getGoodsList(){
		...
		$pageSize = 10;
		$field='goodsId,goodsThums,goodsName,marketPrice,shopPrice';
		$rs = M('Goods')->field($field)->where($where)->limit($pageSize)->order('createTime desc')->select();
		return $rs;
	}
	/**
	 * 所有商品瀑布流
	...
	public function getAjaxGoodsList(){
		...
		$pageSize = 10;
		$field='goodsId,goodsThums,goodsName,marketPrice,shopPrice';
		$rs = M('Goods')->field($field)->where($where)->limit(($page-1)*$pageSize,$pageSize)->order('createTime desc')->select();
		return $rs;
	}
	/**
	 * 商品列表
	 */
	public function getBrandsGoodsList($gBrands = 0){
		$brandsId = (int)I("brandId");
		$brandsId = ($brandsId==0)?$gBrands:$brandsId;
		$where=array(
			'goodsStatus' => 1,
			'goodsFlag' => 1,
			'isSale' => 1,
	...
	public function getGoodsDetails($obj){		
		$goodsId = $obj["goodsId"];
		$rs = M('Goods')->where('goodsFlag = 1')->find($goodsId);
		return $rs;
	}
	/**
	 * 查询商品品牌
	 */
	public function getGoodsBrands($goodsId){
		$sql = "SELECT b.brandDesc,b.brandId,b.brandIco,b.brandName FROM wst_goods g,wst_brands b where g.brandId=b.brandId AND g.goodsId=".$goodsId;
		$rs = $this->query($sql);
		return $rs[0];
	}
	/**
	 * 查询商品品牌
	 */
	public function getGoodsShops($goodsId){
		$sql = "SELECT s.shopId,s.shopName,s.shopImg,s.shopTel FROM wst_goods g,wst_shops s where g.shopId=s.shopId AND g.goodsId=".$goodsId;
		$rs = $this->query($sql);
		return $rs[0];
	}
	/**
	 * getaddtimenewgoods
	 */
	public function getNewtimeGoodlist(){
		return M('Goods')->field('goodsId,goodsThums,goodsName,marketPrice,shopPrice')->ORDER('createTime DESC')->limit(6)->select();
	}
	/**
	 * 获取商品信息-购物车/核对订单用
	 */
