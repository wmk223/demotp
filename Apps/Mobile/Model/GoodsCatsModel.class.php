	...
	*/
	public function queryByList($pid = 0){
		...
		for($i=0;$i<count($rs);$i++){
			$sql = "select b.brandId,b.brandName,b.brandIco from __PREFIX__goods_cat_brands cb,__PREFIX__brands b where cb.brandId=b.brandId and catId=".$rs[$i]['catId'];
	...
	public function queryById($id){
		$m = M('goods_cats');
		return $m->where('catFlag=1 and catId='.$id)->find();
	}
	/**
	 * 获取列表
	 */
	public function queryErByList($pid){
		$m = M('goods_cats');
		$rs = $m->where('catFlag=1 and parentId='.$pid)->select();
		for($i=0;$i<count($rs);$i++){
			$rs[$i]['list'] = $m->where('catFlag=1 and parentId='.$rs[$i]['catId'])->select();
		}
		return $rs;
	}
    /**
