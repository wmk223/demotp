...
function checkCartPay(shopId,goodsId,num,ischk,isBook,goodsAttrId){
	jQuery.post( Think.U('Mobile/Goods/getGoodsStock') ,{goodsId:goodsId,isBook:isBook,goodsAttrId:goodsAttrId},function(data) {
		...
		jQuery.post( Think.U('Mobile/Cart/changeCartGoodsNum') ,{goodsId:goodsId,num:num,ischk:ischk,goodsAttrId:goodsAttrId},function(rsp) {
			if(rsp){
				var totalMoney = 0;	
...
function goToPay(){
	...
	jQuery.post( Think.U('Mobile/Cart/checkCartGoodsStock') ,{},function(data) {
		...
		if(flag){
			location.href = Think.U('Mobile/Orders/checkOrderInfo','rnd='+new Date().getTime());
		}else{
			$("#showwarnmsg").show();
		}
	});
...
function delCatGoods(shopId,goodsId,priceAttrId){
	layer.confirm('您确定要从购物车中删除该商品吗？',{icon: 3, title:'系统提示',offset: '150px'}, function(tips){
		...
		jQuery.post(Think.U('Mobile/Cart/delCartGoods') ,{goodsId:goodsId,goodsAttrId:priceAttrId},function(data) {
			layer.close(ll);
	    	layer.close(tips);
