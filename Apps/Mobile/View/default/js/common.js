...
function checkCart(){
	jQuery.post( Think.U('Mobile/Cart/getCartInfo') ,{"axm":1},function(data) {
		...
		for(var shopId in cart.cartgoods){
			...
			for(var goodsId in shop.shopgoods){
				...
				var url = Think.U('Mobile/Goods/getGoodsDetails','goodsId='+goods.goodsId);
...
function topay(){
	location.href = Think.U('Mobile/Cart/getCartInfo','rnd='+Math.round(Math.random()*10000000));
}

function toChangeCity(){
	location.href = Think.U('Mobile/Index/changeCity');
}

function changeCity(areaId2){
	areaId2 = (areaId2>0)?areaId2:$("#cityId").val();
	
	jQuery.post( Think.U('Mobile/Index/reChangeCity') ,{"city":areaId2,"changeCity":true},function(data) {
...
function getSearchInfo(obj,event){
	...
	if(event.which == 38 || event.which == 40) {
		...
		var surl = Think.U('Mobile/Goods/getKeyList');
    	if(searchType==2){
    		surl = Think.U('Mobile/Shops/getKeyList');
    	}
...
/*************************************用户操作*****************************************/
function login(){
	return location.href= Think.U('Mobile/Users/login');
}
function logout(){
	jQuery.post(Think.U('Mobile/Users/logout'),{},function(rsp) {
		location.reload();
	});
}
function regist(){
	return location.href= Think.U('Mobile/Users/regist');
}
...
//刷新验证码
function getVerify() {
    $('.verifyImg').attr('src',Think.U('Mobile/Users/getVerify','rnd='+Math.random()));
}
function checkLogin(){
...
function addAccess(aid){
	$.post(Think.U('Mobile/Index/access'),{id:aid},function(data,textStatus){
