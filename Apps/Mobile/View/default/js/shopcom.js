...
function queryUnSaleByPage(){
	...
	location.href= Think.U('Mobile/Goods/queryUnSaleByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2);
}
function queryOnSale(){
	var shopCatId1 = $('#shopCatId1').val();
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Mobile/Goods/queryOnSaleByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2);
}
function queryPendding(){
	var shopCatId1 = $('#shopCatId1').val();
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Mobile/Goods/queryPenddingByPage','goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2);
}
function toEditGoods(id,menuId){
	location.href= Think.U('Mobile/Goods/toEdit','umark='+menuId+"&id="+id);
}
function toViewGoods(id){
	$.post(Think.U('Mobile/Goods/getGoodsVerify'),{id:id},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			var verifyCode = json.verifyCode;
			window.open(Think.U('Mobile/Goods/getGoodsDetails','goodsId='+id+"&kcode="+verifyCode));
		}
	});
...
function delGoods(id){
	layer.confirm("您确定要删除该商品？",{icon: 3, title:'系统提示'},function(tips){
		$.post(Think.U('Mobile/Goods/del'),{id:id},function(data,textStatus){
			layer.close(tips);
    		var json = WST.toJson(data);
    		if(json.status=='1'){
...
function batchDel(){
	layer.confirm("您确定要删除这些商品？",{icon: 3, title:'系统提示'},function(){
	      ...
	      var params = {};
	      params.ids = ids;
	      $.post(Think.U('Mobile/Goods/batchDel'),params,function(data,textStatus){
	    		var json = WST.toJson(data);
	    		if(json.status=='1'){
	    			WST.msg('操作成功！', {icon: 1},function(){
	    				location.reload();
...
function sale(v){
	...
	layer.confirm((v==1)?"您确定要上架这些商品？":"您确定要下架这些商品？",{icon: 3, title:'系统提示'},function(tips){
	    ...
	    params.ids = ids;
	    params.isSale= v;
	    $.post(Think.U('Mobile/Goods/sale'),params,function(data,textStatus){
	    	layer.close(loading);
	    	var json = WST.toJson(data);
	    	if(json.status=='1'){
	    		WST.msg('操作成功！', {icon: 1},function(){
...
function goodsSet(type,umark){
	...
	params.ids = ids;
	params.code= type;
	$.post(Think.U('Mobile/Goods/goodsSet'),params,function(data,textStatus){
	    var json = WST.toJson(data);
	    if(json.status=='1'){
	    	WST.msg('操作成功！', {icon: 1},function(){
	    		location.reload();
...
function getShopCatListForGoods(v,id){
	   ...
	   $('#shopCatId2').empty();
	   var html = [];
	   $.post(Think.U('Mobile/ShopsCats/queryByList'),params,function(data,textStatus){
...
function getBrands(catId){
	...
	$.post(Think.U('Mobile/Brands/queryBrandsByCat'),params,function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1' && json.list){
			for(var i=0;i<json.list.length;i++){
				opts = json.list[i];
...
function getCatListForEdit(objId,parentId,t,id){
	   ...
	   var html = [];
	   $.post(Think.U('Mobile/GoodsCats/queryByList'),params,function(data,textStatus){
		    html.push('<option value="">请选择</option>');
			var json = WST.toJson(data);
			if(json.status=='1' && json.list){
				var opts = null;
...
function editGoods(menuId){
	   ...
	   $.post(Think.U('Mobile/Goods/edit'),params,function(data,textStatus){
		   layer.close(loading);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
					if((menuId=='toEditGoods')){
						location.href= Think.U('Mobile/Goods/toEdit','umark=toEditGoods');
					}else{
						location.href=Think.U('Mobile/Goods/'+menuId);
					}
				});
			}else if(json.status=='-2'){
				if(params.isSale==1){
...
function getAttrList(catId){
	...
	$.post(Think.U('Mobile/Attributes/getAttributes'),{catId:catId},function(data,textStatus){
		 var json = WST.toJson(data);
		 var priceAttr = null;
		 if(json.status=='1' && json.list){
			var opts = null;
...
function editGoodsCat(){
	   ...
	   $.post(Think.U('Mobile/ShopsCats/edit'),params,function(data,textStatus){
		   layer.close(loading);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
				   location.href= Think.U('Mobile/ShopsCats/index');
				});
			}else{
...
function delGoodsCat(id){
	layer.confirm("您确定要删除该商品分类吗？",{icon: 3, title:'系统提示'},function(tips){
		layer.load('正在处理，请稍后...', 3);
		layer.close(tips);
		$.post(Think.U('Mobile/ShopsCats/del'),{id:id},function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
					location.reload();
...
function editGoodsCatName(obj){
	var name = $('#');
	$.post(Think.U('Mobile/ShopsCats/editName'),{id:$(obj).attr('dataId'),catName:obj.value},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
...
function editGoodsCatSort(obj){
	var name = $('#');
	$.post(Think.U('Mobile/ShopsCats/editSort'),{id:$(obj).attr('dataId'),catSort:obj.value},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!',{icon: 1,time:500});
		}else{
...
function queryAppraises(){
	...
	var shopCatId2 = $('#shopCatId2').val();
	var goodsName = $('#goodsName').val();
	location.href= Think.U('Mobile/GoodsAppraises/index','umark=GoodsAppraises&goodsName='+goodsName+"&shopCatId1="+shopCatId1+"&shopCatId2="+shopCatId2);
}
...
function showOrder(id){
	layer.open({
	    ...
	    shade: [0.6, '#000'],
	    border: [0],
	    content: [Think.U('Mobile/Orders/getOrderDetails','orderId='+id)],
...
function shopOrderAccept(id){
	layer.confirm('您确定受理该订单吗？', {icon: 3, title:'系统提示'}, function(tips){
	    ...
	    $.post(Think.U('Mobile/Orders/shopOrderAccept'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function batchShopOrderAccept(){
	...
	layer.confirm('您确定受理这些订单吗？', {icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/batchShopOrderAccept'),{orderIds:ids},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderProduce(id){
	layer.confirm('您确定打包该商品吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/shopOrderProduce'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function batchShopOrderProduce(){
	...
	layer.confirm('您确定打包这些商品吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/batchShopOrderProduce'),{orderIds:ids},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
	    	if(json.status>0){
...
function shopOrderDelivery(id){
	layer.confirm('确定正在发货吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/shopOrderDelivery'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function batchShopOrderDelivery(id){
	...
	layer.confirm('您确定这些订单正在发货吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/batchShopOrderDelivery'),{orderIds:ids},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
	    	if(json.status>0){
...
function shopOrderReceipt(id){
	layer.confirm('确定已收货吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
	    $.post(Think.U('Mobile/Orders/shopOrderReceipt'),{orderId:id},function(data){
	    	layer.close(ll);
	    	layer.close(tips);
	    	var json = WST.toJson(data);
			if(json.status>0){
...
function shopOrderRefund(id,type){
	if(type==1){
		layer.confirm('您同意拒收该订单吗？',{icon: 3, title:'系统提示'}, function(tips){
		    var ll = layer.load('数据处理中，请稍候...');
		    $.post(Think.U('Mobile/Orders/shopOrderRefund'),{orderId:id,type:type},function(data){
		    	layer.close(ll);
		    	layer.close(tips);
		    	var json = WST.toJson(data);
				if(json.status>0){
		...
		var w = layer.open({
	        ...
	        yes: function(index, layero){
	        	...
	        	var ll = layer.load('数据处理中，请稍候...');
			    $.post(Think.U('Mobile/Orders/shopOrderRefund'),{orderId:id,type:type,rejectionRemarks:rejectionRemarks},function(data){
			    	layer.close(w);
			    	layer.close(ll);
			    	var json = WST.toJson(data);
					if(json.status>0){
...
function queryOrderPager(statusMark,pcurr){
	...
	$.post(Think.U('Mobile/Orders/queryShopOrders'),param,function(data,textStatus){
			var json = WST.toJson(data);
			var html = new Array();
...
function editPass(){
	   ...
	   $.post(Think.U('Mobile/Users/editPass'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
...
function getCommunitysForShopEdit(){
	  ...
	  $.post(Think.U('Mobile/Areas/getAreaAndCommunitysByList'),{areaId:areaId},function(data,textStatus){
			var json = data;
			if(json.list){
...
function editShop(){
	   ...
	   $.post(Think.U('Mobile/Shops/edit'),params,function(data,textStatus){
			var json = WST.toJson(data);
...
function setShop(){
	   ...
	   $.post(Think.U('Mobile/Shops/editShopCfg'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功!', {icon: 1}, function(){
					location.href=location.href;
...
function logout(){
	jQuery.post(Think.U('Mobile/Shops/logout'),{},function(rsp) {
		location.reload();
	});
}
function checkLogin(){
	jQuery.post(Think.U('Mobile/Shops/checkLoginStatus'),{},function(rsp) {
...
function editAttrCats(type,src){
	...
	var loading = layer.load('正在处理，请稍后...', 3);
	$.post(Think.U('Mobile/AttributeCats/edit'),{umark:src,catName:catName,id:$('#id').val()},function(data,textStatus){
		layer.close(loading);
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!', {icon: 1}, function(){
				if(type==1){
				    ...
				    location.href=Think.U('Mobile/AttributeCats/index');
				}
			});
		}else{
			WST.msg('操作失败!', {icon: 5});
...
function delAttrCat(id){
	layer.confirm("您确定要删除该商品类型及其下的属性吗？",{icon: 3, title:'系统提示'},function(tips){
	    ...
	    layer.close(tips);
	    var params = {};
	    $.post(Think.U('Mobile/AttributeCats/del'),{id:id},function(data,textStatus){
	    	layer.close(loading);
	    	var json = WST.toJson(data);
	    	if(json.status=='1'){
	    		WST.msg('操作成功！', {icon: 1},function(){
...
function getAttrsForAttr(){
	location.href=Think.U("Mobile/Attributes/index",'catId='+$('#catId').val());
}
function toAddAttr(){
...
function editAttrs(){
	...
	var loading = layer.load('正在处理，请稍后...', 3);
	$.post(Think.U('Mobile/attributes/edit'),params,function(data,textStatus){
		layer.close(loading);
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('操作成功!', {icon: 1}, function(){
...
function delAttrs(no,id){
	if(id>0){
		layer.confirm("您确定要删除该商品属性吗？",{icon: 3, title:'系统提示'},function(tips){
		    ...
		    layer.close(tips);
		    var params = {};
		    $.post(Think.U('Mobile/Attributes/del'),{id:id},function(data,textStatus){
		    	layer.close(loading);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
		    		WST.msg('操作成功！', {icon: 1},function(){
...
function batchMessageDel(){
	  layer.confirm("您确定要删除这些消息？",function(){
	        ...
	        var params = {};
	        params.ids = ids;
	        $.post(Think.U('Mobile/Messages/batchDel'),params,function(data,textStatus){
	          var json = WST.toJson(data);
	          if(json.status=='1'){
	        	  WST.msg('操作成功！', {icon: 1},function(){
	              location.reload();
...
function editGoodsBase(fv,goodsId){
	...
	jQuery.post(Think.U('Mobile/Goods/editGoodsBase'),{vfield:fv,goodsId:goodsId,vtext:vtext},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status>0){
...
function changeCatStatus(isShow,id,pid){
	...
	params.pid = pid;
	$.post(Think.U('Mobile/ShopsCats/changeCatStatus'),params,function(data,textStatus){
		location.reload();  
	});
...
function changSaleStatus(goodsId,flag){
	...
	jQuery.post(Think.U('Mobile/Goods/changSaleStatus'),{goodsId:goodsId,tamk:tamk,flag:flag},function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status>0){
			if(tamk == 0){
				tamk = 1;
...
function batchSaveShopCats(){
	...
	$.post(Think.U('Mobile/ShopsCats/batchSaveShopCats'),params,function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status==1){
...
function getShopMsgTips(){
	$.post(Think.U('Mobile/Orders/getShopMsgTips'),{},function(data,textStatus){
		var json = WST.toJson(data);
		for(var i in json){
			if(json[i]>0){
				if(json['100']>0){
