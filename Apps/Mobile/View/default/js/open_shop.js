function visitorShopInit(){
  	   ...
  	   $("#loginName").formValidator({onShow:"",onFocus:"登录账号应该为5-16字母、数字或下划线",onCorrect:""}).inputValidator({min:5,max:16,onError:"登录账号应该为5-16字母、数字或下划线"}).regexValidator({
  	       regExp:"username",dataType:"enum",onError:"登录账号格式错误"
  			...
  			dataType : "json",
  			async : true,
  			url : Think.U('Mobile/Users/checkLoginKey'),
...
function showXiey(id){
		layer.open({
		    ...
		    content: [Think.U('Mobile/Index/toUserProtocol')],
		    btn: ['同意并注册'],
...
function getVerifyCode(){
  		...
  		if(WST.SMS_VERFY=='1'){
  			...
  			html.push('<img style="vertical-align:middle;cursor:pointer;height:39px;" class="verifyImg" src="'+domainURL+'/Apps/Mobile/View/default/images/clickForVerify.png" title="刷新验证码" onclick="javascript:getVerify()"/>');
  			...
  			layer.open({
  			    ...
  			    yes: function(index, layero){
  			    	...
  			    	$.post(Think.U('Mobile/Users/getPhoneVerifyCode'),params,function(data,textStatus){
  			   			var json = WST.toJson(data);
  			   			if(json.status!=1){
...
function visitorOpenShop(){	
	...
	$.post(Think.U('Mobile/Shops/openShop'),params,function(data,textStatus){
	   	var json = WST.toJson(data);
	   	if(json.status>0){
...
function userOpenShop(){
	...
	$.post(Think.U('Mobile/Shops/openShopByUser'),params,function(data,textStatus){
		var json = WST.toJson(data);
		if(json.status=='1'){
			WST.msg('您的开店申请已提交，请等候商城管理员审核!', {icon: 1}, function(){
				location.href=Think.U('Mobile/Orders/queryByPage');
			});
		}else{
			WST.msg(json.msg, {icon: 5});
			getVerify();
