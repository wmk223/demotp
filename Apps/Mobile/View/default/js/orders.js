...
function loadCommunitys(obj){
	...
	jQuery.post(Think.U('Mobile/Communitys/getByDistrict') ,{areaId3:districtId},function(rsp){
...
function loadAddress(addressId){
	$("#address_form").show();	
	jQuery.post(Think.U('Mobile/UserAddress/getUserAddress') ,{addressId:addressId},function(rsp) {
...
function saveAddress(){
	...
	jQuery.post(Think.U('Mobile/UserAddress/edit') ,params,function(data,textStatus){
...
function delAddress(addressId){
	layer.confirm('您确定删除该地址吗？',{icon: 3, title:'系统提示'}, function(tips){
		...
		jQuery.post(Think.U('Mobile/UserAddress/del') ,{id:addressId},function(rsp) {
			layer.close(ll);
	    	layer.close(tips);
			if(rsp){
...
function submitOrder(){
	...
	jQuery.post(Think.U('Mobile/Goods/checkGoodsStock') ,{},function(data) {
		...
		if(flag){
			...
			location.href= Think.U('Mobile/Orders/submitOrder','consigneeId='+consigneeId+"&payway="+payway+"&isself="+isself+"&invoiceClient="+invoiceClient+"&needreceipt="+needreceipt+"&remarks="+remarks+"&requireTime="+requireTime+"&orderunique="+orderunique);
		}else{
			if(goods.isSale<1){
...
function getOrderInfo(orderId){
	window.location = Think.U('Mobile/orders/getOrderInfo','orderId='+orderId);
}
...
function getPayUrl(){
	...
	jQuery.post(Think.U('Mobile/Payments/get'+params.payCode+"URL") ,params,function(data) {
		var json = WST.toJson(data);
		if(json.status==1){
