...
function editAddress(){
	   ...
	   $.post(Think.U('Mobile/UserAddress/edit'),params,function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status>0){
				WST.msg('操作成功!', {icon: 1}, function(){
					location.href = Think.U('Mobile/UserAddress/queryByPage');
				});
			}else{
				WST.msg(' 操作失败!',{icon: 5});
			}
...
function getAreaList(objId,parentId,t,id){
	   ...
	   var html = [];
	   $.post(Think.U('Mobile/Areas/queryByList'),params,function(data,textStatus){
...
function getCommunitys(v,id){
	   ...
	   $('#communityId').empty();
	   var html = [];
	   $.post(Think.U('Mobile/Communitys/getByDistrict'),params,function(data,textStatus){
		    html.push('<option value="">请选择</option>');
			var json = WST.toJson(data);
...
function toEditAddress(id){
	    location.href = Think.U('Mobile/UserAddress/toEdit','id='+id);
	}
	function delAddress(id){
		layer.confirm("您确定要删除该地址吗？",{icon: 3, title:'系统提示'},function(tips){
			var ll = layer.load('数据处理中，请稍候...');
			$.post(Think.U('Mobile/UserAddress/del'),{id:id},function(data,textStatus){
				layer.close(ll);
		    	layer.close(tips);
				var json = WST.toJson(data);
				if(json.status=='1'){
...
function editUser(){
	   ...
	   var ll = layer.load('数据处理中，请稍候...');
	   $.post(Think.U('Mobile/Users/editUser'),params,function(data,textStatus){
		   layer.close(ll);
			var json = WST.toJson(data);
			if(json.status=='1'){
...
function toPay(id){
	...
	var params = {};
	params.orderIds = id;
	jQuery.post(Think.U('Mobile/Orders/checkOrderPay') ,params,function(data) {
		var json = WST.toJson(data);
		if(json.status==1){
			location.href=Think.U('Mobile/Payments/toPay','orderIds='+params.orderIds);
		}else if(json.status==-2){
			...
			setTimeout(function(){
				window.location = Think.U('Mobile/orders/queryDeliveryByPage');
			},1500);
		}
	});
	...
	if(type==1 || type==2){
		var w = layer.open({
	        ...
	        yes: function(index, layero){
	        	...
	        	var ll = layer.load('数据处理中，请稍候...');
			    $.post(Think.U('Mobile/Orders/orderCancel'),{orderId:id,type:1,rejectionRemarks:rejectionRemarks},function(data){
			    	layer.close(w);
			    	layer.close(ll);
			    	var json = WST.toJson(data);
					if(json.status>0){
		...
		layer.confirm('您确定要取消该订单吗？',{icon: 3, title:'系统提示'}, function(tips){
		    var ll = layer.load('数据处理中，请稍候...');
		    $.post(Think.U('Mobile/Orders/orderCancel'),{orderId:id},function(data){
		    	layer.close(ll);
		    	layer.close(tips);
		    	var json = WST.toJson(data);
				if(json.status>0){
...
function appraiseOrder(id){
	layer.open({
	    ...
	    shade: [0.6, '#000'],
	    border: [0],
	    content: [Think.U('Mobile/GoodsAppraises/toAppraise','orderId='+id)],
	    area: ['1020px', ($(window).height() - 50) +'px'],
	    end:function(){
	    	window.location.reload();
	    }
...
function addGoodsAppraises(shopId,goodsId,goodsAttrId,orderId){
	...
	//layer.confirm('您确定要提交该评价吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
		$.post(Think.U('Mobile/GoodsAppraises/addGoodsAppraises'),{shopId:shopId, goodsId:goodsId, goodsAttrId:goodsAttrId,orderId:orderId, goodsScore:goodsScore, timeScore:timeScore, serviceScore:serviceScore, content:content },function(data,textStatus){
			//layer.close(tips);
			layer.close(ll);
			var json = WST.toJson(data);
			if(json.status==1){
...
function orderConfirm(id,type){
	if(type==1){
		layer.confirm('您确定已收货吗？',{icon: 3, title:'系统提示'}, function(tips){
		    var ll = layer.load('数据处理中，请稍候...');
		    $.post(Think.U('Mobile/Orders/orderConfirm'),{orderId:id,type:type},function(data){
		    	layer.close(tips);
		    	layer.close(ll);
		    	var json = WST.toJson(data);
				if(json.status>0){
		...
		var w = layer.open({
	        ...
	        yes: function(index, layero){
	        	...
	        	var ll = layer.load('数据处理中，请稍候...');
			    $.post(Think.U('Mobile/Orders/orderConfirm'),{orderId:id,type:type,rejectionRemarks:rejectionRemarks},function(data){
			    	layer.close(w);
			    	layer.close(ll);
			    	var json = WST.toJson(data);
					if(json.status>0){
...
function getOrdersList(type){
	...
	if(type==1){
		location.href=Think.U('Mobile/Orders/queryPayByPage',params);
	}else if(type==2){
		location.href=Think.U('Mobile/Orders/queryDeliveryByPage',params);
	}else if(type==3){
		location.href=Think.U('Mobile/Orders/queryReceiveByPage',params);
	}else if(type==4){
		location.href=Think.U('Mobile/Orders/queryAppraiseByPage',params);
	}else if(type==5){
		location.href=Think.U('Mobile/Orders/queryCancelOrders',params);
	}else if(type==6){
		location.href=Think.U('Mobile/Orders/queryRefundByPage',params);
	}
...
function getUserMsgTips(){
	$.post(Think.U('Mobile/Orders/getUserMsgTips'),{},function(data,textStatus){
		var json = WST.toJson(data);
		for(var i in json){
			if(i==-2){//未支付
				if(json[i]>0){
...
function queryFavoriteGoods(p){
     ...
     params.p = p;
     $.post(Think.U('Mobile/Favorites/queryGoodsByPage'),params,function(data,textStatus){
       var json = WST.toJson(data);
...
function addCart(goodsId,type,goodsThums){
	...
	jQuery.post(Think.U('Mobile/Cart/addToCartAjax') ,params,function(data) {
		if(type==1){
			location.href= Think.U('Mobile/Cart/toCart');
		}else{
			layer.msg("添加成功!",1,1);
		}
	});
...
function cancelGoodsFavorites(obj,id){
	layer.confirm('您确定取消关注该商品吗？',{icon: 3, title:'系统提示'}, function(tips){
		layer.close(tips);
	    var ll = layer.load('数据处理中，请稍候...');
		jQuery.post(Think.U("Mobile/Favorites/cancelFavorite") ,{id:id,type:0},function(data) {
...
function queryFavoriteShops(p){
	...
	params.p = p;
	$.post(Think.U('Mobile/Favorites/queryShopsByPage'),params,function(data,textStatus){
		layer.close(tips);
	    var json = WST.toJson(data);
	    if(json.status==1 && json.data){
...
function cancelShopFavorites(obj,id){
	layer.confirm('您确定取消关注该店铺吗？',{icon: 3, title:'系统提示'}, function(tips){
		layer.close(tips);
	    var ll = layer.load('数据处理中，请稍候...');
		jQuery.post(Think.U("Mobile/Favorites/cancelFavorite") ,{id:id,type:1},function(data) {
			var json = WST.toJson(data,1);
			if(json.status==1){
				layer.close(ll);
