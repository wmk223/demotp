function getCommunitys(obj){
	...
	params.areaId = vid;
	var html = [];
	$.post(Think.U('Mobile/Communitys/queryByList'),params,function(data,textStatus){
...
function queryGoods(obj,mark){
	...
	params.wstModel = "Mobile";
	params.wstControl = "Goods";
	params.wstAction = "getGoodsList";

	jQuery.post(Think.U('Mobile/Base/getURL') ,params,function(data) {
		var json = WST.toJson(data);
...
function getPriceAttrInfo(id){
	...
	jQuery.post( Think.U('Mobile/Goods/getPriceAttrInfo') ,{goodsId:goodsId,id:id},function(data) {
		var json = WST.toJson(data);
		if(json.id){
...
function getGoodsappraises(goodsId,p){
	...
	params.p = p;
	//加载商品评价
	jQuery.post(Think.U("Mobile/GoodsAppraises/getGoodsappraises") ,params,function(data) {
		var json = WST.toJson(data);
			...
			for(var j=0;j<json.root.length;j++){
			    ...
			    html.push('<tr height="75" style="border:1px dotted #eeeeee;">');
					...
					for(var i=0;i<appraises.goodsScore;i++){
						html.push('<img src="'+domainURL +'/Apps/Mobile/View/default/images/icon_score_yes.png"/>');
					}
					html.push('</div>');
					html.push('<div>时效评分：');
...
function favoriteGoods(id){
	if($('#f0_txt').attr('f')=='0'){
		jQuery.post(Think.U("Mobile/Favorites/favoriteGoods") ,{id:id},function(data) {
...
function favoriteShops(id){
	if($('#f1_txt').attr('f')=='0'){
		jQuery.post(Think.U("Mobile/Favorites/favoriteShops") ,{id:id},function(data) {
			var json = WST.toJson(data,1);
			if(json.status==1){
				$('#f1_txt').html('已关注');
...
function cancelFavorites(id,type){
	jQuery.post(Think.U("Mobile/Favorites/cancelFavorite") ,{id:id,type:type},function(data) {
		var json = WST.toJson(data,1);
		if(json.status==1){
